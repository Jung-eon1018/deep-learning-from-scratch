...
- Q : 하지만 분자는 늘 k번째 출력과 대응되는 k번째 입력의 지수함수의 값인데, 그렇다면 k번째 입력과 직접적인 연관이 있을까? 그렇다면 왜 이렇게 구성된걸까? 그렇다면 소프트맥수 함수에서 입력뉴런의 개수와 출력뉴런의 개수가 늘 같아야 할까? - 이는 소프트맥스 함수를 사용하는 문제가 "분류"라는 것과 연관이 있다. 분류는 늘 "이 입력이 어떤 클래스일 확률이 제일 클까?"를 판단해야 하는데 그렇기 때문에 각 클래스마다 마지막 단계에서 소프트맥스 함수를 사용하여 각 점수를 확률로 변환하는 작업이 필요하다. 이를 수행하는 것이 소프트맥스 함수이다. - 예를 들어 이미지 하나를 보고 토끼, 고양이, 강아지인지 분류를 한다고 하면, 이미지를 입력하고 CNN특징 추출을 거치고 마지막 선형층에서 각각의 점수를 반환하는데 이 점수 벡터를 logits이라고 한다. 이는 클래스 별 점수를 뜻하며 비교 기준이 없기 때문에 마지막 단계에서 소프트맥스 함수를 통해 확률으로 변환하는 것이다.
